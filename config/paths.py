"""
PLAYWRIGHT_CHROMIUM_PATH = Mostra o local da pasta chromium-1194 que você deve por na pasta do projeto, para que ele rode
BASE_PATH = Armazena o caminho principal da base de dados
LOG_DIR = Armazena o path da log
LOG_PATH = Armazena o caminho direto a log
ENV_PATH = Armazena o caminho da .ENV
EXECUTION_MODE = Armazena o txt de controle do projeto
SELENIUM_CHROME = Armazena o caminho da pasta de arquivos .temp
REAL_TIME_UPDATE = Armazena os paths dos diretórios do modo de execução "Atualização em tempo real"
TEMP_DIR = Armazena os paths dos diretórios onde serão alocados arquivos temporarios (Não ficarão no banco de dados)
CLEAR_DIR = Armazena os paths dos diretórios que serão limpos após utilizados
CLEAR_DIR_DATA_RELOAD = Armazena os diretórios que serão utlizados no modo de operação (reload do banco de dados)
DATA_PATHS = Armazena todos os diretórios do banco de dados
FILE_ROUTER = Armazena os diretórios em que o arquivo extraido percorrerá: chave -> valor
FILE_ROUTER_MERGE = Armazena os diretórios em que um arquivo tratado será mesclado com outro(histórico do banco de dados)
PIPELINE_PATHS = Armazena os paths da arquitetura medalhão do banco de dados
"""
...
PLAYWRIGHT_CHROMIUM_PATH = f'{os.path.expanduser("~")} + "/AppData/Local/ms-playwright"'
...
# =========== PCOM =========== # Não está sendo usado

RPA_VALIDA_BOX = Path(f"{BASE_PATH}/Compartilhados/RPA Wave - Validação de Cargas Abastecimento de Lojas")
OUTPUT_PATH_VALIDA_BOX = Path(f"{BASE_PATH}/Compartilhados/Resultado Validação de Carga ABS/status_carga_abs.xlsx")